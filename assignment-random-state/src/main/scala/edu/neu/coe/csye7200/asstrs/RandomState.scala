...
trait RandomState[T] {
  ...
  // 10 points
  def flatMap[U](f: T => RandomState[U]): RandomState[U] = f.apply(this.get) // TO BE IMPLEMENTED
  ...
  // 12 points
  def toStream: LazyList[T] = this.get #:: this.next.toStream // TO BE IMPLEMENTED
}
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double = {x => 1f*(x* 1f/(Long.MaxValue/2 - Long.MinValue/2) - 1f/(Long.MaxValue/2 - Long.MinValue/2)/2)} // TO BE IMPLEMENTED
